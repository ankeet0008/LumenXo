...
export default function GrowingWithUs() {
  return (
    <section className="py-8 sm:py-12 lg:py-16  text-center px-4 sm:px-6 lg:px-8">
      ...
      <div className="overflow-hidden w-full h-24 sm:h-32 lg:h-40 relative bg-purple-100 ">
        {partners.map((partner, index) => (
          <motion.img
            key={index}
            src={partner.logo}
            alt={partner.name}
            className="w-16 h-16 sm:w-24 sm:h-24 lg:w-36 lg:h-36  object-cover absolute top-1/2 transform -translate-y-1/2 shadow-lg border-2 border-white"
            initial={{ x: "100vw" }}
            animate={{ x: "-200px" }}
            transition={{
              duration: 10,
